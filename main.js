...
function registerIpcHandlers() {
  ...
  ipcMain.handle('get-version-info', () => {
    ...
    if (app.isPackaged) {
      // In production, always read from the unpacked package.json
      packageJsonPath = path.join(process.resourcesPath, 'app.asar.unpacked/package.json');
    } else {
      // In development, read from the local package.json
      packageJsonPath = path.join(__dirname, 'package.json');
    }

    try {
      // Always read directly from the package.json file
      const packageJson = JSON.parse(fs.readFileSync(packageJsonPath, 'utf8'));
      appVersion = packageJson.version;
      console.log('Read version from package.json:', { path: packageJsonPath, version: appVersion });
    } catch (err) {
      console.error('Error reading version from package.json:', err);
      // Only use app.getVersion() as a last resort
      appVersion = app.getVersion();
      console.log('Fell back to app.getVersion():', appVersion);
    }

    // Get aitomics version
    let aitomicsVersion;
    try {
      if (app.isPackaged) {
        const aitomicsPackagePath = path.join(process.resourcesPath, 'app.asar.unpacked/node_modules/aitomics/package.json');
        if (fs.existsSync(aitomicsPackagePath)) {
          const aitomicsPackage = JSON.parse(fs.readFileSync(aitomicsPackagePath, 'utf8'));
      ...
      console.error('Error reading aitomics version:', err);
      aitomicsVersion = 'unknown';
    }
